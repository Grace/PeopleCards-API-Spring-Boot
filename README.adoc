...
== Grace's People Cards using Spring Boot - creating a simple API with Java and consuming it in JavaScript
...
Using this project as a reference for your own, will build a web application with Spring Boot that serves a simple API (sans database), a static webpage that consumes it,
and you will use CORS to let the same webpage running outside of Spring Boot on a different port (thanks Python web server!) consume the API.
The 2nd method emulates creating a blackbox API that can be consumed by other web applications running on localhost port 8000.

Note: This project is based on one of the Spring Boot tutorial projects. Language in this file is borrowed from Spring.
...
(from `src/main/java/com/example/springboot/EmployeeController.java`) shows:

====
[source,java]
----
include::initial/src/main/java/com/example/springboot/EmployeeController.java[]
----
====
...
----
$ curl localhost:8080
[You'll see the HTML for index.html here...]
----
====
