...
Using this project as a reference for your own, you will build a web application with Spring Boot that serves a simple API (sans database), a static webpage that consumes it, and you will use CORS to let the same webpage running outside of Spring Boot on a different port (thanks Python web server!) consume the API.
